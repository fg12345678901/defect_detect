...
SEGMENT_MODELS = {
    'magnetic': {'path': 'weights/magnetic_seg.pth', 'classes': 6},
    'phone': {'path': 'weights/phone_seg.pth', 'classes': 4},
    'solar-panel': {'path': 'weights/solar-panel_seg.pth', 'classes': 29},
    'steel': {'path': 'weights/steel_seg.pth', 'classes': 5},
}
...
def task_batch(task):
    ...
    class_counts = {i:0 for i in range(SEGMENT_MODELS[task]['classes'])}
    class_img_map = {}
    out_files = []
    for file in files:
        save_path = UPLOAD_DIR / file.filename
    ...
    return render_template('batch_result.html', stats=stats, class_counts_mapped=class_counts, class_img_map_mapped={}, current_year=2025)
