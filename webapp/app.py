...
app.config['MAX_FILES'] = 1000  # 最大文件数量（自定义配置）
...
def start_batch(task):
    ...
    # 检查文件数量限制
    max_files = app.config.get('MAX_FILES', 1000)
    if len(files) > max_files:
        return jsonify({"error": f"文件数量超过限制，最多支持 {max_files} 个文件"}), 400
    ...
    def worker(path_list):
        ...
        for path in path_list:
            ...
            if np.any(mask > 0):
                ...
                # 先把无缺陷图名记下来，后面一并复制
                class_img_map.setdefault("no_defect", []).append(out_name)
...
def classify_upload():
    year = datetime.now().year
    max_files = app.config.get('MAX_FILES', 1000)
    return render_template("classify.html", task_info=TASK_INFO, current_year=year, max_files=max_files)
...
def request_entity_too_large(error):
    return jsonify({"error": "上传的文件太大，请减少文件数量或大小"}), 413
...
if __name__ == "__main__":
    ...
    print(f"服务器配置:")
    print(f"- 最大上传大小: {max_upload_mb} MB")
    print(f"- 最大文件数量: {max_files} 个")
    print(f"提示: 可通过环境变量 MAX_UPLOAD_MB 和 MAX_FILES 调整限制")
